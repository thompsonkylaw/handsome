...
const getPersonAnalysis = (person, isMarried) => {
  const details = [];
      ...
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
        <div className="max-w-xl w-full bg-white rounded-[2.5rem] shadow-2xl overflow-hidden border border-slate-100">
          <div className="bg-blue-600 p-8 text-white text-center">
            ...
            <p className="text-blue-100 mt-1 font-bold opacity-90 text-sm">2025 / 2026 年度入息及資產審核</p>
          </div>
          
          <div className="p-6 space-y-6">
            <div className="grid grid-cols-2 gap-3">
              <button onClick={() => setIsMarried(false)} className={`py-4 rounded-2xl font-bold border-2 transition-all flex items-center justify-center gap-2 ${!isMarried ? 'bg-blue-600 text-white border-blue-600 shadow-lg' : 'bg-white text-slate-400 border-slate-100 hover:border-blue-200'}`}>
                <User size={18} /> 個人申請
              </button>
              <button onClick={() => setIsMarried(true)} className={`py-4 rounded-2xl font-bold border-2 transition-all flex items-center justify-center gap-2 ${isMarried ? 'bg-blue-600 text-white border-blue-600 shadow-lg' : 'bg-white text-slate-400 border-slate-100 hover:border-blue-200'}`}>
                <Users size={18} /> 夫婦申請
              </button>
            </div>
            ...
            <div className="space-y-4">
              <div className="p-6 bg-slate-50 rounded-3xl border border-slate-100 space-y-4">
                <h3 className="font-black text-slate-800 flex items-center gap-2"><UserCircle className="text-blue-600" /> {isMarried ? "第一位申請人" : "您的個人資料"}</h3>
...
const SummaryList = ({ name, details }) => {
